...
#include <string>
#include <functional>
#include "cuda_rasterizer/network.h"
	...
	const bool debug)
{
  ...
  torch::Tensor out_color = torch::full({COLOR_CHANNELS+8, H, W}, 0.0, float_opts);
	...
	const int n_max)
{
	...
	torch::Tensor final_scale = torch::full({SCALE_DIM * P}, 0, scale_old.options().dtype(torch::kFloat32));
