...
public:
    __device__ ForwardNode(): Node() {}
    __device__ ForwardNode(int local_j, float depth, float alpha) {
    ...
    __device__ void print() {
        printf("FN: local_j %d, depth %f, alpha %f\n", local_j, depth, alpha);
    }
};

class BackwardNode: public Node {
public:
    __device__ BackwardNode() {}
    __device__ BackwardNode(int local_j, float depth, float alpha) {
        this->local_j = local_j;
        this->depth = depth;
        this->alpha = alpha;
        this->next_id = -1;
    ...
    __device__ void print() {
        printf("BW: local_j %d, depth %f, alpha %f\n", local_j, depth, alpha);
    }
};
