	...
	float* __restrict__ dL_dcolors)
{
	...
	auto blend = [&](const Node & node) {
		...
		const int j = node.local_j;
		const float c_d = node.depth;
		const float alpha_ = 1.0f - alpha;

		const float test_T = T * alpha_;
...
#endif
		iter_w = (iter_w - alpha) / alpha_;
		iter_D2 = (iter_D2 - m_d * m_d * alpha) / alpha_;
		iter_D = (iter_D - m_d * alpha) / alpha_;
		...
		for (int ch = 0; ch < C; ch++) bg_dot_dpixel += bg_color[ch] * dL_dpixel[ch];
		dL_dalpha += (-T_final / alpha_) * bg_dot_dpixel;
	...
	for (i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++) {
			...
			const float alpha = min(0.99f, nor_o.w * G); // opa * G
			if (alpha < 1.0f / 255.0f)
				continue;
			
			Node node(j, c_d, alpha);
			bucket.push(node);
